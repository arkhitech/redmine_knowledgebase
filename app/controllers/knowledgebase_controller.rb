class KnowledgebaseController < ApplicationController
  ...
  def index
    ...
    @articles_newest   = KbArticle.limit(summary_limit).order('created_at DESC')
    @articles_updated  = KbArticle.limit(summary_limit).where('created_at <> updated_at').order('updated_at DESC')
    
    #FIXME the following method still requires ALL records to be loaded before being filtered.
    
    a = KbArticle.includes(:viewings).to_a.sort_by(&:view_count)
    a = a.drop(a.count - summary_limit) if a.count > summary_limit
    @articles_popular  = a.reverse
    a = KbArticle.includes(:ratings).to_a.sort_by(&:rated_count)
    a = a.drop(a.count - summary_limit) if a.count > summary_limit
